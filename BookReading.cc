...
int main(int argc, char const *argv[]) {
	if(argc == 1) {
		...
		for(unsigned int i=0; i<dataVector.size(); i++) {
			cout << "PROCESS " << i << endl;
			cout << dataVector[i].tests << " " << dataVector[i].N << " " << dataVector[i].M << " " << dataVector[i].Q << " qVector-->" << endl;

			for(unsigned int j=0; j<dataVector[i].qVector.size(); j++) {
				cout << dataVector[i].qVector[j] << " ";
			}

			cout << endl << "mVector --> " << endl;
			for(unsigned int j=0; j<dataVector[i].mVector.size(); j++) {
				cout << dataVector[i].mVector[j] << " ";
			}
